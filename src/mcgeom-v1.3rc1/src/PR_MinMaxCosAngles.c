...
/* Procedure to compute minimum and maximum cosines of dihedral angles
   of general polyhedron */
...
/* If the polyhedron is of a specific type, say a tet, pyramid, prism
   ...
   argument. So one can call 

   PR_MinMaxCosAngles(rxyz, n, &mincos, &maxcos, 0, 0, 0); 
...
void PR_MinMaxCosAngles(double (*rxyz)[3], int n, double *mincos, 
			double *maxcos, int **rfverts, int *nfv, int nf) {

  int i, j, k, l, found, nedges, iv0, iv1, iv2, sgn=1;
  int feverts[2], (*everts)[2];
  double vec01[3], vec02[3], len1_sqr, len2_sqr, dp, n1[3], n2[3];
  ...
  *maxcos = -99.0;
  ...
  switch (n) {
  case 4:
    Tet_MinMaxCosAngles(rxyz, mincos, maxcos);
    break;
  case 5:
    break;
  case 6:
    break;
  case 8:
    Hex_MinMaxCosAngles(rxyz, mincos, maxcos);
    break;
  default: /* General polyhedron */
	...
	dp = VDot3(n1,n2);
	sgn = (dp < 0) ? -1 : 1;
	dp = sgn*sqrt((dp*dp)/(len1_sqr*len2_sqr));
