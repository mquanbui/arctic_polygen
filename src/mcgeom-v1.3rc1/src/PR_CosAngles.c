...
/* Procedure to compute cosines of dihedral angles of general polyhedron */
/* If the polyhedron is of a specific type, say a tet, pyramid, prism
   ...
   argument. So one can call PR_CosAngles(rxyz, n, cosangs, 0, 0, 0, 0); */
/* For a general polyhedron, connectivity info has to be provided to
   ...
   vertices. ncos is the number of dihedral angles computed (same as
   number of edges of polyhedron */


void PR_CosAngles(double (*rxyz)[3], int n, double *cosangs,
		  int **rfverts, int *nfv, int nf, int *ncos) {

  int i, j, k, l, found, nedges, iv0, iv1, iv2;
  int feverts[2], (*everts)[2];
  double vec01[3], vec02[3], len1_sqr, len2_sqr, n1[3], n2[3], dp;
